                     ...
                     int Nx, int Ny, double *bath, std::vector<double> &posFree, std::vector<double> &posFixed, double perturbation)
{
    ...
    for (int i = 0; i <= n[0]; i++)
    {
        ...
        for (int j = 0; j <= n[1]; j++)
        {
            ...
            double z = (bath[k_low * (Ny + 1) + l_low] * (x_up - x) * (y_up - y) + bath[k_up * (Ny + 1) + l_low] * (x - x_low) * (y_up - y) + bath[k_low * (Ny + 1) + l_up] * (x_up - x) * (y - y_low) + bath[k_up * (Ny + 1) + l_up] * (x - x_low) * (y - y_low)) / (dx * dy) + height0;
